...
from models.competitors import gnn_placeholder

from models.dfnmf import DFNMF  # Fair DeepNMF, and also shallow NMTF
from utils.utils import device, eps
...
class Experiment:
    ...
    def _run_method(self, method, num_clusters, lambda_val, base_layers=None):
        """
        ...
        if method == "nmtf":
            ...
            if base_layers is not None:
                layers_used = base_layers + [num_clusters]
            elif 'base_layers' in self.params:
                layers_used = self.params['base_layers'] + [num_clusters]  # e.g., [40, 40] + [k]
            else:
                layers_used = [num_clusters]  # Default: last layer is set to num_clusters.
            ...
            args_obj = Namespace(
                ...
                layers= layers_used,          #self.params.get('layers', [k]),
                lg=lambda_val
            ...
            model = DFNMF(torch.tensor(self.A_np, dtype=torch.float, device=device),
                          torch.tensor(self.F_real, dtype=torch.float, device=device), args_obj)
        ...
        result = {
            ...
            'loss': loss,
            'layers': layers_used  # always record the layers configuration (or "N/A")
        }
        return result
    ...
    def run_grid_search(self):
        ...
        """
        if self.methods[0] == "deep_fair_nmf":
            ...
            for lambda_val in self.lambda_range:
                for base_layers in arch_options:
                    ...
                    for rep in range(self.repeats):
                        # Pass the architecture candidate to _run_method.
                        result = self._run_method(self.methods[0], num_clusters, lambda_val, base_layers)
...
if __name__ == "__main__":
    ...
    config_real = {
        ...
        'params': {
            ...
            'tol_eig': 1e-8,
            'base_layers': None  # the last layer will be set to k automatically.
        }
    }
    ...
    config_grid = {
        ...
        'lambda_range': [0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1, 5, 10, 50, 100, 25, 500, 1000],
        'repeats': 1,
        'iter': 500,
        'params': {
            'eps': 1e-6,
            ...
            'normalize_laplacian': False,
            'normalize_evec': False,
            'base_layers': [[200, 50, 25, 15], [200, 50, 25], [50, 35, 15], [40, 20] ]   # the last layer will be set to k automatically.
        }
    }
