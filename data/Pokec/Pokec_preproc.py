...
#       - Column 0: Node ID
#       - Column 5: Raw age
#       - Column 6: Class label
# -------------------------------------------------------------------
data = np.genfromtxt(path1 + 'region_job.csv', delimiter=',', skip_header=1, dtype=np.int32)
node_ids    = data[:, 0]
raw_ages    = data[:, 5]
class_labels = data[:, 6]
...
# -------------------------------------------------------------------
E = np.genfromtxt(path1 + 'region_job_relationship.txt', delimiter='\t', dtype=np.int32)
...
sparse.save_npz(path_out + "sparse_Pokec_graph_region_A.npz", A_sparse)
#np.savetxt(path_out + "Pokecgraph_reg1.csv", A, fmt='%d', delimiter=',')
np.savetxt(path_out + "Pokecfeature_reg1.csv", F, fmt='%d', delimiter=',')
np.savetxt(path_out + "Pokeclabel_reg1.csv", labels, fmt='%d', delimiter=',')

print("Pokec pre-processing complete.")
